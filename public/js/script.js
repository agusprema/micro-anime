...
$(document).ready(function(){
    ...
    function sweat_login_list(){
        Swal.fire({
            title: 'Are you sure?',
            text: "you must log in to continue",
            type: 'warning',
            html:
                '<div id="swal2-content" class="mb-2" style="display: block;">you must log in to continue</div>'+
                '<form action="'+ base_url +'login" method="post">' +
                '<div class="form-group">' +
                    '<label for="exampleInputEmail1">Email address</label>' +
                    '<input type="email" class="form-control" id="email" name="email" aria-describedby="emailHelp" placeholder="Enter email" required autocomplete="email" autofocus>' +
                '</div>' +
                '<div class="form-group">' +
                ...
                '<a href="'+ base_url +'login/google" class="btn btn-google btn-user btn-block"><i class="fab fa-google fa-fw"></i> Login with Google</a>' +
                '<a href="'+ base_url +'login/facebook" class="btn btn-facebook btn-user btn-block"><i class="fab fa-facebook-f fa-fw"></i> Login with Facebook</a>' +
                '<hr>' +
                '<div class="form-group text-center">' +
                '<a style="font-size: 0.9rem;" class="btn btn-primary p-2 float-left mr-1" href="'+ base_url +'password/reset">Forgot Password</a>' +
                '<a style="font-size: 0.9rem;" class="btn btn-primary p-2 float-left mr-1" href="'+ base_url +'register">Register</a>' +
                '<button style="font-size: 0.9rem;" type="submit" class="btn p-2 btn-primary float-right">Login</button>' +
                '</div>' +
                '<input type="hidden" name="redirectTo" id="redirectTo" value="'+ $('meta[name="url-full"]').attr("content") +'">'+
                '<input type="hidden" name="_token" id="_token" value="'+ $('meta[name="csrf-token"]').attr("content") +'">'+
                '</form>',
